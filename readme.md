...
- **Безопасность**: Проверяет Basic Auth (`user:password`) или JWT, добавляет в черный список после 5 неудачных попыток.
