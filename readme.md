...
2. **Сгенерируйте сертификаты** (для HTTPS и QUIC):
   Например, с помощью `openssl`:
   ```bash
   openssl req -x509 -newkey rsa:2048 -keyout key.pem -out cert.pem -days 365 -nodes
   ```
3. **Настройте конфиг**:
   Файл `config.toml` в корне проекта:
   ```toml
	...
	quic_port = 444
	# Где лежит сгенерированный сертификат из п.2
	cert_path = "cert.pem"
	# Где лежит сгенерированный ключ из п.2
	key_path = "key.pem"
	...
	[[locations]]
	path = "/static"
	response = "Сходу в Торговый Пост за барахлом!"
   ```
